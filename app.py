...
def account_me():
    u=_current_user()
    if not u:return jsonify(authenticated=False,user=None,usage=0,limit=30,unlimited=False,consents=None,server_version='6.10.0'),401
    ...
    payload={'authenticated':True,'user':u,'usage':_usage_for(u['id']),'limit':None if u.get('is_admin') else _plan_limit(u['plan']),'unlimited':bool(u.get('is_admin')),'consents':_consent_status(u['id']),'server_version':'6.10.0','auth_token':token,'auto_login':auto_login,'idle_timeout_seconds':AUTH_IDLE_SECONDS}
...
def account_register():
    ...
    try:
        ...
        return _auth_json_response({'ok':True,'user':user,'auth_token':token,'verification_required':bool(code),'server_version':'6.10.0','auto_login':False,'idle_timeout_seconds':AUTH_IDLE_SECONDS},token=token,persistent=False)
...
def account_login():
    ...
    try:
        ...
        return _auth_json_response({
            'ok':True,'user':user,'auth_token':token,'server_version':'6.10.0','auto_login':auto_login,'idle_timeout_seconds':AUTH_IDLE_SECONDS,
...
def account_login_status():
    try:
        ...
        return jsonify(ok=True,database=True,secure_cookie=bool(app.config.get('SESSION_COOKIE_SECURE')),version='6.10.0',idle_timeout_seconds=AUTH_IDLE_SECONDS)
    except Exception:
        logging.exception('login status database failed')
        return jsonify(ok=False,database=False,error='로그인 데이터베이스 연결 실패',version='6.10.0'),503
...
def health():
    try:
        with ENGINE.connect() as con:con.execute(text('SELECT 1')).scalar_one()
        return jsonify(ok=True,version='6.10.0',database='postgresql' if DB_URL.startswith('postgresql') else 'sqlite')
    except Exception as exc:
        logging.exception('health database check failed')
        return jsonify(ok=False,version='6.10.0',database='unavailable',error='database connection failed'),503
