...
def _init_db():
    ...
    with ENGINE.begin() as con:
        ...
        if DB_URL.startswith('postgresql'):
            ...
            additions={'email_key':'VARCHAR(255)','email_verified':'INTEGER NOT NULL DEFAULT 0','verification_code_hash':'VARCHAR(128)','verification_expires_at':'VARCHAR(40)','registration_ip_hash':'VARCHAR(128)','reset_code_hash':'VARCHAR(128)','reset_expires_at':'VARCHAR(40)','reset_requested_at':'VARCHAR(40)','auth_version':'INTEGER NOT NULL DEFAULT 1','plan_started_at':'VARCHAR(40)','plan_expires_at':'VARCHAR(40)'}
...
def _current_user():
    ...
    with ENGINE.connect() as con:
        row=_row_dict(con.execute(text('SELECT id,email,display_name,plan,created_at,last_login_at,email_verified,auth_version,plan_started_at,plan_expires_at FROM users WHERE id=:id'),{'id':uid}).first())
    if row:
...
def health():
    try:
        ...
        return jsonify(ok=True,version='6.2.1',database='postgresql' if DB_URL.startswith('postgresql') else 'sqlite')
    except Exception as exc:
        logging.exception('health database check failed')
        return jsonify(ok=False,version='6.2.1',database='unavailable',error='database connection failed'),503
