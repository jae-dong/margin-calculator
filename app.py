import base64,json,os,re,hashlib,time,threading,sqlite3,logging
import json5
...
def _init_db():
    ...
    with ENGINE.begin() as con:
        for q in (users_sql,snapshot_sql,usage_sql): con.execute(text(q))
        if DB_URL.startswith('sqlite'):
            cols=[r[1] for r in con.exec_driver_sql('PRAGMA table_info(user_snapshots)').fetchall()]
            if 'version' not in cols: con.exec_driver_sql('ALTER TABLE user_snapshots ADD COLUMN version INTEGER NOT NULL DEFAULT 1')
...
def _current_user():
    ...
    with ENGINE.connect() as con:return _row_dict(con.execute(text('SELECT id,email,display_name,plan,created_at,last_login_at FROM users WHERE id=:id'),{'id':uid}).first())
...
def account_register():
    ...
    try:
        with ENGINE.begin() as con: uid=int(con.execute(text('INSERT INTO users(email,password_hash,display_name,plan,created_at,last_login_at) VALUES(:e,:p,:n,:pl,:c,:l) RETURNING id'),{'e':email,'p':generate_password_hash(password),'n':name,'pl':'free','c':now,'l':now}).first()[0])
        session.clear();session.permanent=True;session['user_id']=uid;return jsonify(ok=True,user=_current_user())
    except IntegrityError:return jsonify(error='이미 가입된 이메일입니다.'),409
...
def account_change_password():
    ...
    if len(new)<8:return jsonify(error='새 비밀번호는 8자 이상이어야 합니다.'),400
...
def admin_summary():
    ...
    if u['email'].lower() not in admins:return jsonify(error='운영자 권한이 없습니다.'),403
