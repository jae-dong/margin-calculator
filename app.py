...
def _db_url():
    if DATABASE_URL:
        return DATABASE_URL.replace('postgres://','postgresql+psycopg://',1) if DATABASE_URL.startswith('postgres://') else DATABASE_URL
...
ENGINE=create_engine(_db_url(),pool_pre_ping=True,pool_recycle=300,future=True,
 connect_args={'timeout':30,'check_same_thread':False} if _db_url().startswith('sqlite') else {})
