import base64,json,os,re,hashlib,time,threading,sqlite3,logging,secrets,smtplib
import json5
...
def _init_db():
    ...
    with ENGINE.begin() as con:
        for q in (users_sql,snapshot_sql,usage_sql,reg_sql): con.execute(text(q))
        if DB_URL.startswith('postgresql'):
            ...
            additions={'email_key':'VARCHAR(255)','email_verified':'INTEGER NOT NULL DEFAULT 0','verification_code_hash':'VARCHAR(128)','verification_expires_at':'VARCHAR(40)','registration_ip_hash':'VARCHAR(128)'}
...
def _current_user():
    ...
    with ENGINE.connect() as con: row=_row_dict(con.execute(text('SELECT id,email,display_name,plan,created_at,last_login_at,email_verified FROM users WHERE id=:id'),{'id':uid}).first())
    if row:
        row['is_admin']=row['email'].lower() in _admins();row['email_verified']=bool(row.get('email_verified'))
    return row
...
def account_me():
    u=_current_user(); return jsonify(authenticated=bool(u),user=u,usage=(_usage_for(u['id']) if u else 0),limit=(_plan_limit(u['plan']) if u else 20))
...
def account_login():
    ...
    with ENGINE.connect() as con:row=_row_dict(con.execute(text('SELECT * FROM users WHERE email=:e'),{'e':email}).first())
    if not row or not check_password_hash(row['password_hash'],password):return jsonify(error='이메일 또는 비밀번호가 맞지 않습니다.'),401
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:con.execute(text('UPDATE users SET last_login_at=:n WHERE id=:i'),{'n':now,'i':row['id']})
    session.clear();session.permanent=True;session['user_id']=row['id'];return jsonify(ok=True,user=_current_user())
...
def account_change_password():
    ...
    with ENGINE.begin() as con:con.execute(text('UPDATE users SET password_hash=:p WHERE id=:i'),{'p':generate_password_hash(new),'i':u['id']})
    return jsonify(ok=True)
...
def admin_summary():
    u,err=_require_user()
    if err:return err
    if u['email'].lower() not in _admins():return jsonify(error='운영자 권한이 없습니다.'),403
    with ENGINE.connect() as con:
        users=con.execute(text('SELECT COUNT(*) FROM users')).scalar_one();paid=con.execute(text("SELECT COUNT(*) FROM users WHERE plan<>'free'")).scalar_one();usage=con.execute(text('SELECT COALESCE(SUM(analysis_count),0) FROM monthly_usage WHERE month_key=:m'),{'m':_month_key()}).scalar_one()
    return jsonify(users=users,paid_users=paid,month_analysis=usage,database='PostgreSQL' if DATABASE_URL else 'SQLite 테스트')
...
def health():
    try:
        ...
        return jsonify(ok=True,version='6.1.0',database='postgresql' if DB_URL.startswith('postgresql') else 'sqlite')
    except Exception as exc:
        logging.exception('health database check failed')
        return jsonify(ok=False,version='6.1.0',database='unavailable',error='database connection failed'),503
