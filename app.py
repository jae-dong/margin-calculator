...
# 이용약관·개인정보 처리 및 클라우드 자동 저장 동의 이력 버전
TERMS_VERSION='2026-07-27-v1'
PRIVACY_VERSION='2026-07-27-v2'
CLOUD_AUTO_SAVE_CONSENT_VERSION='2026-07-27-v1'
...
def _security_headers(response):
    ...
    if request.path.startswith('/api/') or request.path in {'/','/index.html','/sw.js','/version.json','/manifest.json','/privacy','/terms','/account-delete','/privacy.html','/terms.html','/account-delete.html'}:
...
def _init_db():
    ...
    persistent_login_sql="""CREATE TABLE IF NOT EXISTS persistent_login_tokens(
      token_hash VARCHAR(128) PRIMARY KEY,user_id INTEGER NOT NULL,auth_version INTEGER NOT NULL,
      created_at VARCHAR(40) NOT NULL,last_activity_at VARCHAR(40) NOT NULL,revoked_at VARCHAR(40),
      user_agent_hash VARCHAR(128),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)"""
    beta_feedback_sql="""CREATE TABLE IF NOT EXISTS beta_feedback(
      id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,user_id INTEGER NOT NULL,
      category VARCHAR(20) NOT NULL,severity VARCHAR(20) NOT NULL,title VARCHAR(120) NOT NULL,
      description TEXT NOT NULL,steps TEXT,diagnostics TEXT,app_version VARCHAR(40),device TEXT,page VARCHAR(80),
      status VARCHAR(20) NOT NULL DEFAULT 'new',admin_note TEXT,created_at VARCHAR(40) NOT NULL,updated_at VARCHAR(40) NOT NULL,
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)"""
    if DB_URL.startswith('sqlite'):
        beta_feedback_sql=beta_feedback_sql.replace('INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY','INTEGER PRIMARY KEY AUTOINCREMENT')
    app_settings_sql="""CREATE TABLE IF NOT EXISTS app_runtime_settings(
      setting_key VARCHAR(80) PRIMARY KEY,setting_value TEXT NOT NULL DEFAULT '',updated_by INTEGER,updated_at VARCHAR(40) NOT NULL)"""
    beta_testers_sql="""CREATE TABLE IF NOT EXISTS beta_testers(
      user_id INTEGER PRIMARY KEY,status VARCHAR(20) NOT NULL DEFAULT 'selected',note TEXT,
      invited_at VARCHAR(40) NOT NULL,joined_at VARCHAR(40),last_seen_at VARCHAR(40),completed_at VARCHAR(40),
      test_count INTEGER NOT NULL DEFAULT 0,updated_by INTEGER,updated_at VARCHAR(40) NOT NULL,
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)"""
    beta_checkins_sql="""CREATE TABLE IF NOT EXISTS beta_daily_checkins(
      user_id INTEGER NOT NULL,checkin_date VARCHAR(10) NOT NULL,checked_at VARCHAR(40) NOT NULL,
      app_version VARCHAR(40),device TEXT,checklist_json TEXT,note TEXT,
      PRIMARY KEY(user_id,checkin_date),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)"""
    client_error_sql="""CREATE TABLE IF NOT EXISTS client_error_logs(
      id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,user_id INTEGER,error_type VARCHAR(40) NOT NULL,
      message TEXT NOT NULL,stack TEXT,source VARCHAR(500),page VARCHAR(100),app_version VARCHAR(40),device TEXT,
      status VARCHAR(20) NOT NULL DEFAULT 'new',admin_note TEXT,occurrence_count INTEGER NOT NULL DEFAULT 1,
      first_seen_at VARCHAR(40) NOT NULL,last_seen_at VARCHAR(40) NOT NULL,created_at VARCHAR(40) NOT NULL,updated_at VARCHAR(40) NOT NULL,
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE SET NULL)"""
    if DB_URL.startswith('sqlite'):
        client_error_sql=client_error_sql.replace('INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY','INTEGER PRIMARY KEY AUTOINCREMENT')
    ai_usage_sql="""CREATE TABLE IF NOT EXISTS ai_usage_logs(
      id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,user_id INTEGER,kind VARCHAR(40),model VARCHAR(80),
      input_tokens INTEGER NOT NULL DEFAULT 0,output_tokens INTEGER NOT NULL DEFAULT 0,total_tokens INTEGER NOT NULL DEFAULT 0,
      estimated_usd REAL NOT NULL DEFAULT 0,cached INTEGER NOT NULL DEFAULT 0,created_at VARCHAR(40) NOT NULL,
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE SET NULL)"""
    if DB_URL.startswith('sqlite'):
        ai_usage_sql=ai_usage_sql.replace('INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY','INTEGER PRIMARY KEY AUTOINCREMENT')
    with ENGINE.begin() as con:
        for q in (users_sql,snapshot_sql,usage_sql,reg_sql,audit_sql,notice_sql,consent_sql,auth_session_sql,persistent_login_sql,beta_feedback_sql,app_settings_sql,beta_testers_sql,beta_checkins_sql,client_error_sql,ai_usage_sql): con.execute(text(q))
        if DB_URL.startswith('postgresql'):
            ...
            for q in (
                ...
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS suspended_at VARCHAR(40)"
            ): con.execute(text(q))
        else:
            cols=[r[1] for r in con.exec_driver_sql('PRAGMA table_info(users)').fetchall()]
            additions={'email_key':'VARCHAR(255)','email_verified':'INTEGER NOT NULL DEFAULT 0','verification_code_hash':'VARCHAR(128)','verification_expires_at':'VARCHAR(40)','registration_ip_hash':'VARCHAR(128)','reset_code_hash':'VARCHAR(128)','reset_expires_at':'VARCHAR(40)','reset_requested_at':'VARCHAR(40)','auth_version':'INTEGER NOT NULL DEFAULT 1','plan_started_at':'VARCHAR(40)','plan_expires_at':'VARCHAR(40)','role':"VARCHAR(20) NOT NULL DEFAULT 'user'",'account_status':"VARCHAR(20) NOT NULL DEFAULT 'active'",'member_number':'INTEGER','failed_login_count':'INTEGER NOT NULL DEFAULT 0','locked_until':'VARCHAR(40)','suspension_reason':'TEXT','suspended_at':'VARCHAR(40)'}
        ...
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_persistent_login_user_activity ON persistent_login_tokens(user_id,last_activity_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_beta_feedback_user_created ON beta_feedback(user_id,created_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_beta_feedback_status_created ON beta_feedback(status,created_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_beta_testers_status_seen ON beta_testers(status,last_seen_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_beta_checkins_date ON beta_daily_checkins(checkin_date,checked_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_client_errors_status_seen ON client_error_logs(status,last_seen_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_client_errors_user_seen ON client_error_logs(user_id,last_seen_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_ai_usage_created ON ai_usage_logs(created_at)'))
        con.execute(text('CREATE INDEX IF NOT EXISTS ix_ai_usage_user_created ON ai_usage_logs(user_id,created_at)'))
        defaults={
          'latest_version':'6.19.0','minimum_version':'6.12.2','update_mode':'optional',
          'update_message':'Android 베타 앱 패키징과 네이티브 연결 지원이 추가되었습니다.',
          'update_url':'/','beta_program_name':'리셀 PICK 비공개 베타','beta_target_count':'15',
          'release_checklist_json':'{}','release_checklist_note':''
        }
        now_setting=datetime.utcnow().isoformat(timespec='seconds')+'Z'
        for key,value in defaults.items():
            exists=con.execute(text('SELECT setting_key FROM app_runtime_settings WHERE setting_key=:k'),{'k':key}).first()
            if not exists:
                con.execute(text('INSERT INTO app_runtime_settings(setting_key,setting_value,updated_by,updated_at) VALUES(:k,:v,NULL,:t)'),{'k':key,'v':value,'t':now_setting})
...
AUTH_IDLE_SECONDS=60*60
AUTH_TOKEN_MAX_AGE=AUTH_IDLE_SECONDS
...
PERSISTENT_LOGIN_COOKIE_NAME='resell_pick_login_v1'
DEVICE_TOKEN_HEADER='X-Resell-Pick-Device'
LOGIN_RESUME_HEADER='X-Resell-Pick-Resume-V2'
PERSISTENT_LOGIN_HEADER='X-Resell-Pick-Login'
...
def _issue_restore_token(uid,auth_version,session_id=None):
    ...
    return _restore_serializer().dumps(payload)

def _decode_restore_token(token):
    token=str(token or '').strip()
    if not token:return None,None,None
    try:
        payload=_restore_serializer().loads(token,max_age=RESTORE_TOKEN_MAX_AGE)
        return int(payload.get('uid')),int(payload.get('av') or 1),str(payload.get('sid') or '')
    except (BadSignature,SignatureExpired,TypeError,ValueError):
        return None,None,None

def _iso_utc(dt=None):
    return (dt or datetime.utcnow()).isoformat(timespec='seconds')+'Z'
...
def _delete_auth_session(sid):
    ...
    try:
        with ENGINE.begin() as con:
            con.execute(text('UPDATE auth_sessions SET revoked_at=:r WHERE session_id=:s'),{'r':_iso_utc(),'s':str(sid)})
    except Exception:logging.exception('auth session revoke failed')
...
def _validate_auth_session(sid,uid=None,auth_version=None,touch=True):
    ...
    last=_parse_utc(row.get('last_activity_at'))
    if not last or (datetime.utcnow()-last).total_seconds()>AUTH_IDLE_SECONDS:
        _delete_auth_session(sid);return None
    if touch and (datetime.utcnow()-last).total_seconds()>=30:
        now=_iso_utc()
        with ENGINE.begin() as con:con.execute(text('UPDATE auth_sessions SET last_activity_at=:n WHERE session_id=:s'),{'n':now,'s':str(sid)})
...
def _persistent_login_hash(token):
    raw=str(token or '').strip()
    if not raw:return ''
    # Opaque 48-byte random tokens are stored only as SHA-256 hashes.
    # This remains valid across Render workers and SECRET_KEY rotations.
    return hashlib.sha256(raw.encode('utf-8')).hexdigest()

def _request_persistent_login_token():
    token=str(request.headers.get(PERSISTENT_LOGIN_HEADER) or '').strip()
    if not token and request.is_json:
        data=request.get_json(silent=True) or {}
        token=str(data.get('persistent_login_token') or '').strip()
    if not token:token=str(request.cookies.get(PERSISTENT_LOGIN_COOKIE_NAME) or '').strip()
    return token

def _create_persistent_login_token(uid,auth_version):
    raw=secrets.token_urlsafe(48);digest=_persistent_login_hash(raw);now=_iso_utc()
    ua=str(request.headers.get('User-Agent') or '')
    ua_hash=hashlib.sha256((str(app.config['SECRET_KEY'])+'|persistent-ua|'+ua).encode()).hexdigest()
    with ENGINE.begin() as con:
        con.execute(text('INSERT INTO persistent_login_tokens(token_hash,user_id,auth_version,created_at,last_activity_at,revoked_at,user_agent_hash) VALUES(:h,:u,:a,:c,:l,NULL,:ua)'),{
            'h':digest,'u':int(uid),'a':int(auth_version or 1),'c':now,'l':now,'ua':ua_hash})
    return raw

def _validate_persistent_login_token(token,touch=True):
    digest=_persistent_login_hash(token)
    if not digest:return None
    with ENGINE.connect() as con:
        row=_row_dict(con.execute(text('SELECT token_hash,user_id,auth_version,created_at,last_activity_at,revoked_at,user_agent_hash FROM persistent_login_tokens WHERE token_hash=:h'),{'h':digest}).first())
    if not row or row.get('revoked_at'):return None
    last=_parse_utc(row.get('last_activity_at'))
    if not last or (datetime.utcnow()-last).total_seconds()>AUTH_IDLE_SECONDS:
        with ENGINE.begin() as con:con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE token_hash=:h'),{'r':_iso_utc(),'h':digest})
        return None
    user=_load_auth_user(row.get('user_id'),row.get('auth_version'))
    if not user:
        with ENGINE.begin() as con:con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE token_hash=:h'),{'r':_iso_utc(),'h':digest})
        return None
    if touch and (datetime.utcnow()-last).total_seconds()>=15:
        now=_iso_utc()
        with ENGINE.begin() as con:con.execute(text('UPDATE persistent_login_tokens SET last_activity_at=:n WHERE token_hash=:h AND revoked_at IS NULL'),{'n':now,'h':digest})
        row['last_activity_at']=now
    return row

def _ensure_persistent_login_token(uid,auth_version):
    current=_request_persistent_login_token()
    row=_validate_persistent_login_token(current,touch=False) if current else None
    if row and int(row.get('user_id') or 0)==int(uid) and int(row.get('auth_version') or 0)==int(auth_version or 1):
        return current
    return _create_persistent_login_token(uid,auth_version)

def _revoke_persistent_login_token(token):
    digest=_persistent_login_hash(token)
    if not digest:return
    with ENGINE.begin() as con:con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE token_hash=:h'),{'r':_iso_utc(),'h':digest})

def _revoke_all_persistent_login_tokens(uid):
    with ENGINE.begin() as con:con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE user_id=:u AND revoked_at IS NULL'),{'r':_iso_utc(),'u':int(uid)})

def _issue_auth_token(uid,auth_version,session_id=None,auto_login=False):
    payload={'uid':int(uid),'av':int(auth_version or 1)}
    if session_id:payload['sid']=str(session_id)
    payload['al']=1 if auto_login else 0
    return _auth_serializer().dumps(payload)

def _decode_auth_token(token):
    token=str(token or '').strip()
    if not token:return None,None,None,False
    try:
        payload=_auth_serializer().loads(token,max_age=AUTH_TOKEN_MAX_AGE)
        return int(payload.get('uid')),int(payload.get('av') or 1),str(payload.get('sid') or ''),bool(payload.get('al'))
    except (BadSignature,SignatureExpired,TypeError,ValueError):
...
def _ensure_session_record(uid,auth_version,sid=None,auto_login=False):
    # sid가 있으면 만료·삭제된 세션을 새 세션으로 되살리지 않습니다.
    if sid:return _validate_auth_session(sid,uid,auth_version)
    # v6.9.x에서 넘어온 기존 로그인만 한 번 새 세션 레코드로 승격합니다.
    sid=_create_auth_session(uid,auth_version,auto_login)
    return {'session_id':sid,'user_id':int(uid),'auth_version':int(auth_version or 1),'auto_login':bool(auto_login),'last_activity_at':_iso_utc()}

def _current_user():
    """Resolve the user from one opaque DB-backed 1-hour login token.

    The token is stored in localStorage and an HttpOnly cookie.  It is independent
    of Flask session cookies, worker processes and SECRET_KEY rotations.
    """
    persistent_token=_request_persistent_login_token()
    if persistent_token:
        prow=_validate_persistent_login_token(persistent_token,touch=True)
        if prow:
            user=_load_auth_user(prow.get('user_id'),prow.get('auth_version'))
            if user:
                user['auto_login']=True
                user['auth_session_id']=''
                return user
    # Signed token fallback only for one-time migration from v6.12.1.
    token_uid,token_auth,_token_sid,token_auto=_request_auth_token()
    if token_uid:
        row=_load_auth_user(token_uid,token_auth)
        if row:
            row['auto_login']=bool(token_auto)
            row['auth_session_id']=''
            return row
    ...
    if cookie_uid:
        ...
        if row:
            row['auto_login']=bool(session.get('auto_login'))
            row['auth_session_id']=''
            return row
        session.clear()
    return None

def _auth_json_response(payload,status=200,token=None,persistent=False,device_token=None,login_resume_token=None,persistent_login_token=None):
    ...
    if token:response.set_cookie(AUTH_COOKIE_NAME,token,**kwargs)
    if persistent_login_token:
        response.set_cookie(PERSISTENT_LOGIN_COOKIE_NAME,persistent_login_token,**kwargs)
    elif not token:
        response.delete_cookie(PERSISTENT_LOGIN_COOKIE_NAME,path='/')
    # Remove obsolete recovery cookies only.
    ...
    response.headers['X-Resell-Pick-Auth']='active' if (token or persistent_login_token) else 'none'
    return response
...
def _analysis_access():
    u=_current_user()
    if not u:
        return None,(jsonify(error='회원가입 후 이용해 주세요.',code='login_required'),401)
...
def _require_user():
    u=_current_user(); return (u,None) if u else (None,(jsonify(error='회원가입 후 이용해 주세요.',code='login_required'),401))

# v6.9.9: 회원가입·로그인 전에는 계정 생성/복구 외 모든 API 기능을 차단합니다.
_PUBLIC_API_PATHS={
    '/api/account/me',
...
@app.before_request
def require_member_for_app_api():
    path=(request.path or '').rstrip('/') or '/'
    if request.method=='OPTIONS' or not path.startswith('/api/') or path in _PUBLIC_API_PATHS:
        return None
    if _current_user():
        return None
    return jsonify(error='회원가입 후 이용해 주세요.',code='login_required'),401

@app.route('/api/account/me',methods=['GET','POST'])
def account_me():
    u=_current_user()
    if not u:return jsonify(authenticated=False,user=None,usage=0,limit=30,unlimited=False,consents=None,server_version='6.19.0',code='login_required'),401
    auth_version=int(u.get('auth_version') or session.get('auth_version') or 1)
    session.clear();session.permanent=True
    session['user_id']=int(u['id']);session['auth_version']=auth_version;session['auto_login']=True
    persistent_token=_ensure_persistent_login_token(u['id'],auth_version)
    payload={'authenticated':True,'user':u,'usage':_usage_for(u['id']),'limit':None if u.get('is_admin') else _plan_limit(u['plan']),'unlimited':bool(u.get('is_admin')),'consents':_consent_status(u['id']),'server_version':'6.19.0','persistent_login_token':persistent_token,'auto_login':True,'idle_timeout_seconds':AUTH_IDLE_SECONDS}
    return _auth_json_response(payload,persistent=True,persistent_login_token=persistent_token)
...
def account_resume_v2():
    ...
    payload={'ok':True,'authenticated':True,'user':user,'usage':_usage_for(uid),'limit':None if user.get('is_admin') else _plan_limit(user['plan']),'unlimited':bool(user.get('is_admin')),'consents':_consent_status(uid),'server_version':'6.19.0','auth_token':auth_token,'restore_token':restore_token,'device_token':device_token,'login_resume_token':login_resume_token,'idle_timeout_seconds':AUTH_IDLE_SECONDS,'message':'로그인 상태를 복원했습니다.'}
...
def account_heartbeat():
    u=_current_user()
    if not u:return jsonify(error='로그인 시간이 만료되었습니다.',code='login_required'),401
    auth_version=int(u.get('auth_version') or session.get('auth_version') or 1)
    persistent_token=_ensure_persistent_login_token(u['id'],auth_version)
    session.clear();session.permanent=True
    session['user_id']=int(u['id']);session['auth_version']=auth_version;session['auto_login']=True
    return _auth_json_response({'ok':True,'authenticated':True,'persistent_login_token':persistent_token,'server_version':'6.19.0','idle_timeout_seconds':AUTH_IDLE_SECONDS},persistent=True,persistent_login_token=persistent_token)
...
def account_restore():
    ...
    login_resume_token=_issue_login_resume_token(uid,auth_version)
    payload={'ok':True,'authenticated':True,'user':user,'usage':_usage_for(uid),'limit':None if user.get('is_admin') else _plan_limit(user['plan']),'unlimited':bool(user.get('is_admin')),'consents':_consent_status(uid),'server_version':'6.19.0','auth_token':auth_token,'restore_token':new_restore_token,'device_token':device_token,'login_resume_token':login_resume_token,'auto_login':auto_login,'idle_timeout_seconds':AUTH_IDLE_SECONDS,'message':'로그인 상태를 복원했습니다.'}
    return _auth_json_response(payload,token=auth_token,persistent=True,device_token=device_token,login_resume_token=login_resume_token)
...
def account_register():
    ...
    try:
        ...
        auto_login=False
        session.clear();session.permanent=True;session['user_id']=uid;session['auth_version']=1;session['auto_login']=False
        user=_load_auth_user(uid,1);persistent_token=_create_persistent_login_token(uid,1)
        return _auth_json_response({'ok':True,'authenticated':True,'user':user,'persistent_login_token':persistent_token,'verification_required':bool(code),'server_version':'6.19.0','auto_login':True,'idle_timeout_seconds':AUTH_IDLE_SECONDS},persistent=True,persistent_login_token=persistent_token)
...
@app.post('/api/account/cloud-autosave-consent')
def account_cloud_autosave_consent():
    u,err=_require_user()
    if err:return err
    d=request.get_json(silent=True) or {};enabled=bool(d.get('enabled'))
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z';ip_hash=_client_ip_hash();ua=str(request.headers.get('User-Agent') or '')[:500]
    consent_type='cloud_auto_save_enabled' if enabled else 'cloud_auto_save_disabled'
    with ENGINE.begin() as con:
        con.execute(text('INSERT INTO user_consents(user_id,consent_type,document_version,accepted_at,ip_hash,user_agent) VALUES(:u,:t,:v,:a,:ip,:ua)'),{'u':u['id'],'t':consent_type,'v':CLOUD_AUTO_SAVE_CONSENT_VERSION,'a':now,'ip':ip_hash,'ua':ua})
    return jsonify(ok=True,enabled=enabled,document_version=CLOUD_AUTO_SAVE_CONSENT_VERSION,recorded_at=now)
...
def account_login():
    ...
    password=str(d.get('password') or '');auto_login=bool(d.get('auto_login'))
    ...
    if row.get('account_status')=='suspended':
        ...
        message='이 계정은 이용이 제한되었습니다.'
        if reason: message+=f' 사유: {reason}'
        message+=' 관리자에게 문의해 주세요.'
        return jsonify(error=message,code='account_suspended'),403
    ...
    try:
        ...
        session.clear();session.permanent=True
        session['user_id']=int(row['id']);session['auth_version']=auth_version;session['auto_login']=auto_login
        user=_load_auth_user(row['id'],auth_version)
        if not user:raise RuntimeError('authenticated user lookup failed')
        user['auto_login']=auto_login;user['auth_session_id']=''
        persistent_token=_create_persistent_login_token(row['id'],auth_version)
        return _auth_json_response({
            'ok':True,'authenticated':True,'user':user,'persistent_login_token':persistent_token,'server_version':'6.19.0','auto_login':True,'idle_timeout_seconds':AUTH_IDLE_SECONDS,
            'message':'관리자 계정으로 로그인했습니다.' if user.get('is_admin') else '로그인했습니다.'
        },persistent=True,persistent_login_token=persistent_token)
    except Exception:
...
def account_login_status():
    try:
        ...
        return jsonify(ok=True,database=True,secure_cookie=bool(app.config.get('SESSION_COOKIE_SECURE')),version='6.19.0',idle_timeout_seconds=AUTH_IDLE_SECONDS)
    except Exception:
        logging.exception('login status database failed')
        return jsonify(ok=False,database=False,error='로그인 데이터베이스 연결 실패',version='6.19.0'),503
...
def account_reset_password():
    ...
    with ENGINE.begin() as con:
        con.execute(text('UPDATE users SET password_hash=:p,reset_code_hash=NULL,reset_expires_at=NULL,auth_version=auth_version+1 WHERE id=:i'),{'p':generate_password_hash(new),'i':row['id']})
        con.execute(text('DELETE FROM auth_sessions WHERE user_id=:i'),{'i':row['id']})
        con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE user_id=:i AND revoked_at IS NULL'),{'r':_iso_utc(),'i':row['id']})
...
def account_logout():
    current=_request_persistent_login_token()
    if current:
        try:_revoke_persistent_login_token(current)
        except Exception:logging.exception('persistent logout revoke failed')
    session.clear()
    return _clear_auth_response({'ok':True})
...
def account_logout_all():
    ...
    with ENGINE.begin() as con:
        ...
        con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE user_id=:i AND revoked_at IS NULL'),{'r':_iso_utc(),'i':u['id']})
    av=int(row[0]);session.clear();session.permanent=True
    session['user_id']=u['id'];session['auth_version']=av;session['auto_login']=auto_login
    persistent_token=_create_persistent_login_token(u['id'],av)
    return _auth_json_response({'ok':True,'authenticated':True,'persistent_login_token':persistent_token,'auto_login':True,'server_version':'6.19.0','message':'현재 기기를 제외한 모든 기기에서 로그아웃했습니다.'},persistent=True,persistent_login_token=persistent_token)
...
def account_export():
    ...
    with ENGINE.connect() as con:
        ...
        beta_checkins=[dict(r._mapping) for r in con.execute(text('SELECT checkin_date,checked_at,app_version,checklist_json,note FROM beta_daily_checkins WHERE user_id=:i ORDER BY checkin_date'),{'i':u['id']}).fetchall()]
    payload={}
    if snap and snap.get('payload'):
        try:payload=json.loads(snap['payload'])
        except Exception:payload={}
    out={'service':'리셀 PICK','exported_at':datetime.utcnow().isoformat(timespec='seconds')+'Z','profile':profile or {},'usage':usages,'beta_checkins':beta_checkins,'cloud':{'version':(snap or {}).get('version',0),'updated_at':(snap or {}).get('updated_at'),'payload':payload}}
...
def account_change_password():
    ...
    with ENGINE.begin() as con:
        ...
        row2=con.execute(text('SELECT auth_version FROM users WHERE id=:i'),{'i':u['id']}).first()
        con.execute(text('DELETE FROM auth_sessions WHERE user_id=:i'),{'i':u['id']})
        con.execute(text('UPDATE persistent_login_tokens SET revoked_at=:r WHERE user_id=:i AND revoked_at IS NULL'),{'r':_iso_utc(),'i':u['id']})
    av=int(row2[0]);auto_login=bool(session.get('auto_login') or u.get('auto_login'))
    session.clear();session.permanent=True;session['user_id']=u['id'];session['auth_version']=av;session['auto_login']=auto_login
    persistent_token=_create_persistent_login_token(u['id'],av)
    return _auth_json_response({'ok':True,'authenticated':True,'persistent_login_token':persistent_token,'server_version':'6.19.0','auto_login':True},persistent=True,persistent_login_token=persistent_token)

def _delete_account_rows(con,user_row):
    uid=int(user_row['id']);email_key=_canonical_email(user_row.get('email') or '')
    # SQLite의 외래키 설정 여부와 관계없이 계정 연결 데이터를 명시적으로 정리합니다.
    for table in ('beta_daily_checkins','beta_testers','beta_feedback','user_consents','persistent_login_tokens','auth_sessions','monthly_usage','user_snapshots'):
        con.execute(text(f'DELETE FROM {table} WHERE user_id=:u'),{'u':uid})
    con.execute(text('UPDATE client_error_logs SET user_id=NULL WHERE user_id=:u'),{'u':uid})
    con.execute(text('UPDATE ai_usage_logs SET user_id=NULL WHERE user_id=:u'),{'u':uid})
    con.execute(text('UPDATE admin_audit_logs SET target_user_id=NULL WHERE target_user_id=:u'),{'u':uid})
    if email_key:
        con.execute(text('DELETE FROM registration_attempts WHERE email_key=:e'),{'e':email_key})
    con.execute(text('DELETE FROM users WHERE id=:u'),{'u':uid})
...
def account_delete():
    ...
    with ENGINE.connect() as con:row=_row_dict(con.execute(text('SELECT id,email,password_hash,role,member_number FROM users WHERE id=:i'),{'i':u['id']}).first())
    if not row or not check_password_hash(row['password_hash'],password):return jsonify(error='비밀번호가 맞지 않습니다.'),401
    if row.get('role')=='admin' or int(row.get('member_number') or 0)==100000:return jsonify(error='관리자 계정은 공개 화면에서 삭제할 수 없습니다. 먼저 다른 계정에 관리자 권한을 이전해 주세요.'),403
    with ENGINE.begin() as con:_delete_account_rows(con,row)
    session.clear();return _clear_auth_response({'ok':True,'message':'회원정보와 계정 연결 데이터가 삭제되었습니다.'})

_PUBLIC_DELETE_RATE={};_PUBLIC_DELETE_RATE_LOCK=threading.Lock()
def _public_delete_rate_allowed():
    key=_client_ip_hash();now=time.time();window=10*60
    with _PUBLIC_DELETE_RATE_LOCK:
        recent=[x for x in _PUBLIC_DELETE_RATE.get(key,[]) if now-x<window]
        if len(recent)>=5:
            _PUBLIC_DELETE_RATE[key]=recent;return False
        recent.append(now);_PUBLIC_DELETE_RATE[key]=recent;return True

@app.post('/api/public/account-delete')
def public_account_delete():
    if not _public_delete_rate_allowed():return jsonify(error='요청이 너무 많습니다. 10분 후 다시 시도해 주세요.'),429
    d=request.get_json(silent=True) or {}
    identifier,is_member_number=_normalize_login_identifier(d.get('identifier') or d.get('email') or '')
    password=str(d.get('password') or '')
    confirmation=str(d.get('confirmation') or '').strip()
    if not identifier or not password:return jsonify(error='이메일 또는 회원번호와 비밀번호를 입력해 주세요.'),400
    if confirmation!='회원탈퇴':return jsonify(error='확인 문구에 회원탈퇴를 정확히 입력해 주세요.'),400
    with ENGINE.connect() as con:
        if is_member_number:
            row=_row_dict(con.execute(text('SELECT id,email,password_hash,role,member_number FROM users WHERE member_number=:m'),{'m':int(identifier)}).first())
        else:
            canonical=_canonical_email(identifier)
            row=_row_dict(con.execute(text('SELECT id,email,password_hash,role,member_number FROM users WHERE email_key=:e OR LOWER(email)=:r ORDER BY id ASC LIMIT 1'),{'e':canonical,'r':identifier.lower()}).first())
    if not row or not check_password_hash(row.get('password_hash') or '',password):
        return jsonify(error='계정 정보가 일치하지 않습니다.'),401
    if row.get('role')=='admin' or int(row.get('member_number') or 0)==100000:
        return jsonify(error='관리자 계정은 이 페이지에서 삭제할 수 없습니다.'),403
    with ENGINE.begin() as con:_delete_account_rows(con,row)
    session.clear();return _clear_auth_response({'ok':True,'message':'회원정보와 계정 연결 데이터 삭제가 완료되었습니다.'})

@app.get('/api/mobile-shell-config')
def mobile_shell_config():
    base=(os.getenv('PUBLIC_APP_URL') or os.getenv('RENDER_EXTERNAL_URL') or request.url_root).rstrip('/')
    return jsonify(
      ok=True,app='리셀 PICK',web_version='6.19.0',android_version='1.0.0',
      base_url=base,support_url='https://open.kakao.com/o/gCdVxSFi',
      privacy_url=base+'/privacy',terms_url=base+'/terms',account_delete_url=base+'/account-delete',
      minimum_android_api=26,target_android_api=36,native_bridge='v1'
    )

@app.get('/api/public/legal-status')
def public_legal_status():
    base=(os.getenv('PUBLIC_APP_URL') or os.getenv('RENDER_EXTERNAL_URL') or request.url_root).rstrip('/')
    return jsonify(ok=True,version='6.19.0',privacy_url=base+'/privacy',terms_url=base+'/terms',account_delete_url=base+'/account-delete')
...
def beta_feedback_list():
    u,err=_require_user()
    if err:return err
    limit=min(20,max(1,int(request.args.get('limit') or 8)))
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text('''SELECT id,category,severity,title,description,steps,status,admin_note,created_at,updated_at
          FROM beta_feedback WHERE user_id=:u ORDER BY id DESC LIMIT :l'''),{'u':u['id'],'l':limit}).fetchall()]
    return jsonify(items=rows)

@app.post('/api/beta/feedback')
def beta_feedback_create():
    u,err=_require_user()
    if err:return err
    d=request.get_json(silent=True) or {}
    category=str(d.get('category') or 'bug').strip().lower()
    severity=str(d.get('severity') or 'medium').strip().lower()
    title=str(d.get('title') or '').strip()[:120]
    description=str(d.get('description') or '').strip()[:5000]
    steps=str(d.get('steps') or '').strip()[:3000]
    diagnostics=d.get('diagnostics') if isinstance(d.get('diagnostics'),dict) else {}
    if category not in {'bug','ux','idea','data','other'}:return jsonify(error='올바른 의견 종류를 선택해 주세요.'),400
    if severity not in {'low','medium','high','blocker'}:return jsonify(error='올바른 중요도를 선택해 주세요.'),400
    if len(title)<2:return jsonify(error='제목을 2자 이상 입력해 주세요.'),400
    if len(description)<3:return jsonify(error='내용을 3자 이상 입력해 주세요.'),400
    diag_raw=json.dumps(diagnostics,ensure_ascii=False,separators=(',',':'))[:12000]
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        fid=int(con.execute(text('''INSERT INTO beta_feedback(user_id,category,severity,title,description,steps,diagnostics,app_version,device,page,status,created_at,updated_at)
          VALUES(:u,:c,:s,:t,:d,:p,:g,:v,:x,:pg,'new',:n,:n) RETURNING id'''),{
          'u':u['id'],'c':category,'s':severity,'t':title,'d':description,'p':steps,'g':diag_raw,
          'v':str(d.get('app_version') or '')[:40],'x':str(d.get('device') or '')[:1000],
          'pg':str(d.get('page') or '')[:80],'n':now}).first()[0])
    return jsonify(ok=True,id=fid,message='베타 의견이 접수되었습니다. 확인 후 개선에 반영하겠습니다.')

@app.get('/api/admin/beta-feedback')
def admin_beta_feedback_list():
    ...
    if status in {'new','reviewing','resolved'}:
        where='WHERE f.status=:s';params['s']=status
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text(f'''SELECT f.id,f.user_id,u.member_number,u.email,u.display_name,f.category,f.severity,f.title,f.description,f.steps,f.diagnostics,f.app_version,f.device,f.page,f.status,f.admin_note,f.created_at,f.updated_at
          FROM beta_feedback f JOIN users u ON u.id=f.user_id {where} ORDER BY CASE f.status WHEN 'new' THEN 0 WHEN 'reviewing' THEN 1 ELSE 2 END,f.id DESC LIMIT :lim'''),params).fetchall()]
        counts=dict(con.execute(text("SELECT status,COUNT(*) AS cnt FROM beta_feedback GROUP BY status")).fetchall())
    return jsonify(items=rows,summary={'new':int(counts.get('new',0)),'reviewing':int(counts.get('reviewing',0)),'resolved':int(counts.get('resolved',0)),'total':sum(int(v) for v in counts.values())})

@app.post('/api/admin/beta-feedback/<int:fid>/status')
def admin_beta_feedback_status(fid):
    admin,err=_require_admin()
    if err:return err
    d=request.get_json(silent=True) or {}
    status=str(d.get('status') or '').strip().lower()
    note=str(d.get('admin_note') or '').strip()[:2000]
    if status not in {'new','reviewing','resolved'}:return jsonify(error='올바른 처리 상태를 선택해 주세요.'),400
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=con.execute(text('SELECT user_id,title FROM beta_feedback WHERE id=:i'),{'i':fid}).first()
        if not row:return jsonify(error='의견을 찾을 수 없습니다.'),404
        con.execute(text('UPDATE beta_feedback SET status=:s,admin_note=:n,updated_at=:t WHERE id=:i'),{'s':status,'n':note or None,'t':now,'i':fid})
    _audit(admin['id'],'update_beta_feedback',int(row[0]),json.dumps({'feedback_id':fid,'status':status,'title':row[1]},ensure_ascii=False))
    return jsonify(ok=True,status=status,updated_at=now)

@app.get('/api/admin/beta-feedback-export.csv')
def admin_beta_feedback_export():
    admin,err=_require_admin()
    if err:return err
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text('''SELECT f.id,u.member_number,u.email,u.display_name,f.category,f.severity,f.title,f.description,f.steps,f.app_version,f.page,f.status,f.admin_note,f.created_at,f.updated_at
          FROM beta_feedback f JOIN users u ON u.id=f.user_id ORDER BY f.id DESC''')).fetchall()]
    out=StringIO();out.write('\ufeff');w=csv.writer(out)
    w.writerow(['번호','회원번호','이메일','이름','종류','중요도','제목','내용','재현순서','앱버전','화면','상태','관리자메모','등록일','수정일'])
    for r in rows:w.writerow([r.get(k,'') for k in ['id','member_number','email','display_name','category','severity','title','description','steps','app_version','page','status','admin_note','created_at','updated_at']])
    _audit(admin['id'],'export_beta_feedback',None,f'{len(rows)}건')
    return Response(out.getvalue(),mimetype='text/csv; charset=utf-8',headers={'Content-Disposition':'attachment; filename=resell_pick_beta_feedback.csv','Cache-Control':'no-store'})
...
def admin_users():
    ...
    if q:
        ...
        params['q']='%'+q+'%'
    sql=f'''SELECT id,member_number,email,display_name,plan,role,account_status,email_verified,created_at,last_login_at,plan_started_at,plan_expires_at,suspension_reason,suspended_at
...
def admin_users_export():
    ...
    if err:return err
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text('''SELECT member_number,email,display_name,plan,role,account_status,email_verified,created_at,last_login_at,plan_started_at,plan_expires_at,suspension_reason,suspended_at FROM users ORDER BY id ASC''')).fetchall()]
    out=StringIO();out.write('\ufeff')
    writer=csv.writer(out)
    writer.writerow(['회원번호','이메일','이름','이용권','권한','계정상태','이메일인증','가입일','최근로그인','이용권시작','이용권만료','이용제한사유','이용제한일시'])
    for r in rows:
        writer.writerow([r.get('member_number') or '',r.get('email') or '',r.get('display_name') or '',r.get('plan') or 'free',r.get('role') or 'user',r.get('account_status') or 'active','예' if r.get('email_verified') else '아니오',r.get('created_at') or '',r.get('last_login_at') or '',r.get('plan_started_at') or '',r.get('plan_expires_at') or '',r.get('suspension_reason') or '',r.get('suspended_at') or ''])
...
def admin_user_plan(uid):
    ...
    action=str(d.get('action') or 'replace').lower()
    try: days=int(d.get('days') or 0)
    except (TypeError,ValueError): return jsonify(error='선물할 일수를 숫자로 입력해 주세요.'),400
    if plan not in {'free','pro','proplus'}:return jsonify(error='올바른 이용권을 선택해 주세요.'),400
    if action not in {'replace','gift'}:return jsonify(error='올바른 적용 방식을 선택해 주세요.'),400
    if plan=='free': days=0
    elif days<1 or days>3660:return jsonify(error='선물할 기간은 1~3660일 범위로 입력해 주세요.'),400
    now_dt=datetime.utcnow();now=now_dt.isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=_row_dict(con.execute(text('SELECT role,plan,plan_started_at,plan_expires_at FROM users WHERE id=:i'),{'i':uid}).first())
        if not row:return jsonify(error='회원을 찾을 수 없습니다.'),404
        if row.get('role')=='admin':return jsonify(error='관리자 계정은 자동 무제한이므로 이용권을 변경할 수 없습니다.'),400
        if plan=='free':
            start=None;expiry=None
        else:
            base=now_dt
            keep_started=False
            if action=='gift' and row.get('plan')==plan and row.get('plan_expires_at'):
                try:
                    current_expiry=datetime.fromisoformat(str(row['plan_expires_at']).replace('Z','+00:00')).replace(tzinfo=None)
                    if current_expiry>now_dt:
                        base=current_expiry;keep_started=True
                except Exception: pass
            start=(row.get('plan_started_at') if keep_started and row.get('plan_started_at') else now)
            expiry=(base+timedelta(days=days)).isoformat(timespec='seconds')+'Z'
        con.execute(text('UPDATE users SET plan=:p,plan_started_at=:s,plan_expires_at=:e WHERE id=:i'),{'p':plan,'s':start,'e':expiry,'i':uid})
    remaining=0
    if expiry:
        try:remaining=max(0,(datetime.fromisoformat(expiry.replace('Z','+00:00')).replace(tzinfo=None).date()-now_dt.date()).days)
        except Exception:remaining=days
    detail={'plan':plan,'days':days,'action':action,'expires_at':expiry,'remaining_days':remaining}
    _audit(admin['id'],'gift_plan' if action=='gift' and plan!='free' else 'change_plan',uid,json.dumps(detail,ensure_ascii=False))
    return jsonify(ok=True,plan=plan,days=days,action=action,plan_expires_at=expiry,days_remaining=remaining)
...
def admin_user_status(uid):
    ...
    if uid==admin['id']:return jsonify(error='현재 로그인한 관리자 계정은 제한할 수 없습니다.'),400
    if status=='suspended' and len(reason)<2:return jsonify(error='이용 제한 사유를 2자 이상 입력해 주세요.'),400
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=con.execute(text('SELECT role FROM users WHERE id=:i'),{'i':uid}).first()
        if not row:return jsonify(error='회원을 찾을 수 없습니다.'),404
        if row[0]=='admin':return jsonify(error='다른 관리자 계정은 이 화면에서 제한할 수 없습니다.'),400
        con.execute(text('''UPDATE users
          SET account_status=:s,auth_version=auth_version+1,
              suspension_reason=:r,suspended_at=:t
          WHERE id=:i'''),{
            's':status,
            'r':reason if status=='suspended' else None,
            't':now if status=='suspended' else None,
            'i':uid
        })
    _audit(admin['id'],'restrict_user' if status=='suspended' else 'restore_user',uid,json.dumps({'status':status,'reason':reason},ensure_ascii=False))
    return jsonify(ok=True,status=status,reason=reason if status=='suspended' else None)
...
def _runtime_settings():
    with ENGINE.connect() as con:
        rows=con.execute(text('SELECT setting_key,setting_value FROM app_runtime_settings')).fetchall()
    return {str(r[0]):str(r[1] or '') for r in rows}

def _save_runtime_settings(values,admin_id):
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        for key,value in values.items():
            row=con.execute(text('SELECT setting_key FROM app_runtime_settings WHERE setting_key=:k'),{'k':key}).first()
            if row:
                con.execute(text('UPDATE app_runtime_settings SET setting_value=:v,updated_by=:u,updated_at=:t WHERE setting_key=:k'),{'v':str(value),'u':admin_id,'t':now,'k':key})
            else:
                con.execute(text('INSERT INTO app_runtime_settings(setting_key,setting_value,updated_by,updated_at) VALUES(:k,:v,:u,:t)'),{'k':key,'v':str(value),'u':admin_id,'t':now})

def _kst_today():
    return (datetime.utcnow()+timedelta(hours=9)).date().isoformat()

def _beta_progress(con,user_id):
    dates=[str(r[0]) for r in con.execute(text('SELECT checkin_date FROM beta_daily_checkins WHERE user_id=:u ORDER BY checkin_date'),{'u':user_id}).fetchall()]
    total=len(dates);today=_kst_today();checked_today=today in set(dates)
    streak=0
    if dates:
        parsed=[datetime.strptime(x,'%Y-%m-%d').date() for x in dates]
        cursor=parsed[-1];present=set(parsed)
        while cursor in present:
            streak+=1;cursor-=timedelta(days=1)
    return {'checkin_days':total,'required_days':14,'checked_today':checked_today,'streak_days':streak,
      'first_checkin_date':dates[0] if dates else None,'last_checkin_date':dates[-1] if dates else None,
      'progress_percent':min(100,round(total/14*100)),'app_checkin_complete':total>=14}

RELEASE_CHECKLIST_ITEMS=[
  ('critical_errors','치명적 오류 처리 완료'),('login_camera','로그인·카메라 실기기 검증'),
  ('privacy_terms','이용약관·개인정보처리방침 최종본'),('public_legal','개인정보·약관·계정삭제 공개 URL 확인'),('account_delete','회원탈퇴·데이터 삭제 검증'),
  ('store_assets','앱 아이콘·스크린샷·설명 준비'),('tester_joined','테스터 12명 이상 참여'),
  ('aab_uploaded','Play Console AAB 업로드'),('closed_test','비공개 테스트 기간 충족')
]

def _redact_error_text(value,limit=6000):
    text_value=str(value or '')[:limit]
    text_value=re.sub(r'(?i)(password|비밀번호|auth[_ -]?token|login[_ -]?key|authorization)(["\'\s:=]+)[^\s,;}]+',r'\1\2[숨김]',text_value)
    text_value=re.sub(r'(?i)bearer\s+[A-Za-z0-9._~-]+','Bearer [숨김]',text_value)
    return text_value

def _record_client_error(user_id,error_type,message,stack='',source='',page='',app_version='',device='',status='new'):
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    message=_redact_error_text(message,2000);stack=_redact_error_text(stack,8000)
    source=_redact_error_text(source,500);page=_redact_error_text(page,100);app_version=_redact_error_text(app_version,40);device=_redact_error_text(device,1000)
    if not message:return None
    cutoff=(datetime.utcnow()-timedelta(minutes=5)).isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=con.execute(text('''SELECT id,occurrence_count FROM client_error_logs
          WHERE ((user_id=:u) OR (user_id IS NULL AND :u IS NULL)) AND message=:m AND page=:p AND app_version=:v
          AND last_seen_at>=:c ORDER BY id DESC LIMIT 1'''),{'u':user_id,'m':message,'p':page,'v':app_version,'c':cutoff}).first()
        if row:
            con.execute(text('UPDATE client_error_logs SET occurrence_count=:n,last_seen_at=:t,updated_at=:t,status=CASE WHEN status=\'resolved\' THEN \'new\' ELSE status END WHERE id=:i'),{'n':int(row[1] or 1)+1,'t':now,'i':int(row[0])})
            return int(row[0])
        rid=con.execute(text('''INSERT INTO client_error_logs(user_id,error_type,message,stack,source,page,app_version,device,status,admin_note,occurrence_count,first_seen_at,last_seen_at,created_at,updated_at)
          VALUES(:u,:et,:m,:s,:src,:p,:v,:d,:st,NULL,1,:t,:t,:t,:t) RETURNING id'''),{'u':user_id,'et':str(error_type or 'javascript')[:40],'m':message,'s':stack or None,'src':source or None,'p':page or None,'v':app_version or None,'d':device or None,'st':status,'t':now}).first()[0]
        return int(rid)

def _record_ai_usage(user,meta):
    if not meta or not isinstance(meta,dict):return
    try:
        with ENGINE.begin() as con:
            con.execute(text('''INSERT INTO ai_usage_logs(user_id,kind,model,input_tokens,output_tokens,total_tokens,estimated_usd,cached,created_at)
              VALUES(:u,:k,:m,:i,:o,:tt,:c,:ca,:t)'''),{
                'u':(user or {}).get('id'),'k':str(meta.get('kind') or 'analysis')[:40],'m':str(meta.get('model') or '')[:80],
                'i':int(meta.get('input_tokens') or 0),'o':int(meta.get('output_tokens') or 0),'tt':int(meta.get('total_tokens') or 0),
                'c':float(meta.get('estimated_usd') or 0),'ca':1 if meta.get('cached') else 0,
                't':str(meta.get('occurred_at') or datetime.utcnow().isoformat(timespec='seconds')+'Z')[:40]})
    except Exception:logging.exception('ai usage log failed')

@app.get('/api/app-config')
def app_config():
    s=_runtime_settings();current=str(request.args.get('version') or '').strip()
    latest=s.get('latest_version','6.19.0');minimum=s.get('minimum_version','6.12.2');mode=s.get('update_mode','optional')
    update_available=bool(current and _version_tuple(current)<_version_tuple(latest))
    force_update=bool(mode=='required' and current and _version_tuple(current)<_version_tuple(minimum or latest))
    return jsonify(latest_version=latest,minimum_version=minimum,update_mode=mode,update_available=update_available,force_update=force_update,
      update_message=s.get('update_message',''),update_url=s.get('update_url','/'),beta_program_name=s.get('beta_program_name','리셀 PICK 비공개 베타'),
      beta_target_count=int(s.get('beta_target_count') or 15),server_version='6.19.0')

@app.get('/api/admin/app-settings')
def admin_app_settings():
    admin,err=_require_admin()
    if err:return err
    return jsonify(settings=_runtime_settings())

@app.post('/api/admin/app-settings')
def admin_app_settings_save():
    admin,err=_require_admin()
    if err:return err
    d=request.get_json(silent=True) or {};mode=str(d.get('update_mode') or 'optional').lower()
    latest=str(d.get('latest_version') or '6.19.0').strip()[:40];minimum=str(d.get('minimum_version') or latest).strip()[:40]
    if mode not in {'off','optional','required'}:return jsonify(error='업데이트 방식을 확인해 주세요.'),400
    if not re.fullmatch(r'\d+(?:\.\d+){1,3}',latest) or not re.fullmatch(r'\d+(?:\.\d+){1,3}',minimum):return jsonify(error='버전은 6.19.0 형식으로 입력해 주세요.'),400
    try:beta_target=max(12,min(100,int(d.get('beta_target_count') or 15)))
    except (TypeError,ValueError):return jsonify(error='베타 목표 인원은 12~100명 사이 숫자로 입력해 주세요.'),400
    values={'latest_version':latest,'minimum_version':minimum,'update_mode':mode,
      'update_message':str(d.get('update_message') or '').strip()[:1000],
      'update_url':str(d.get('update_url') or '/').strip()[:500] or '/',
      'beta_program_name':str(d.get('beta_program_name') or '리셀 PICK 비공개 베타').strip()[:100],
      'beta_target_count':beta_target}
    _save_runtime_settings(values,admin['id']);_audit(admin['id'],'update_app_settings',None,json.dumps(values,ensure_ascii=False))
    return jsonify(ok=True,settings=_runtime_settings())
...
def beta_status():
    ...
    if err:return err
    with ENGINE.connect() as con:
        row=_row_dict(con.execute(text('SELECT status,note,invited_at,joined_at,last_seen_at,completed_at,test_count FROM beta_testers WHERE user_id=:u'),{'u':u['id']}).first())
        progress=_beta_progress(con,u['id'])
    return jsonify(selected=bool(row and row.get('status')!='removed'),tester=row or {'status':'not_selected','test_count':0},progress=progress)

@app.post('/api/beta/checkin')
def beta_daily_checkin():
    u,err=_require_user()
    if err:return err
    d=request.get_json(silent=True) or {};now=datetime.utcnow().isoformat(timespec='seconds')+'Z';day=_kst_today()
    checklist=d.get('checklist') if isinstance(d.get('checklist'),dict) else {}
    checklist={str(k)[:40]:bool(v) for k,v in checklist.items()}
    note=str(d.get('note') or '').strip()[:1000];version=str(d.get('app_version') or '')[:40];device=str(d.get('device') or '')[:1000]
    with ENGINE.begin() as con:
        tester=_row_dict(con.execute(text('SELECT status,joined_at FROM beta_testers WHERE user_id=:u'),{'u':u['id']}).first())
        if not tester or tester.get('status')=='removed':return jsonify(error='현재 베타테스터 선정 대상이 아닙니다.'),403
        existing=con.execute(text('SELECT user_id FROM beta_daily_checkins WHERE user_id=:u AND checkin_date=:d'),{'u':u['id'],'d':day}).first()
        payload=json.dumps(checklist,ensure_ascii=False)
        if existing:
            con.execute(text('UPDATE beta_daily_checkins SET checked_at=:t,app_version=:v,device=:dv,checklist_json=:c,note=:n WHERE user_id=:u AND checkin_date=:d'),{'t':now,'v':version or None,'dv':device or None,'c':payload,'n':note or None,'u':u['id'],'d':day})
        else:
            con.execute(text('INSERT INTO beta_daily_checkins(user_id,checkin_date,checked_at,app_version,device,checklist_json,note) VALUES(:u,:d,:t,:v,:dv,:c,:n)'),{'u':u['id'],'d':day,'t':now,'v':version or None,'dv':device or None,'c':payload,'n':note or None})
        status='active' if tester.get('status')=='selected' else tester.get('status')
        joined=tester.get('joined_at') or now
        con.execute(text('UPDATE beta_testers SET status=:s,joined_at=:j,last_seen_at=:t,updated_at=:t WHERE user_id=:u'),{'s':status,'j':joined,'t':now,'u':u['id']})
        progress=_beta_progress(con,u['id'])
    return jsonify(ok=True,checkin_date=day,already_checked=bool(existing),progress=progress)

@app.post('/api/beta/ping')
def beta_ping():
    u,err=_require_user()
    if err:return err
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=_row_dict(con.execute(text('SELECT status,joined_at,test_count FROM beta_testers WHERE user_id=:u'),{'u':u['id']}).first())
        if not row or row.get('status')=='removed':return jsonify(selected=False,status='not_selected')
        status='active' if row.get('status')=='selected' else row.get('status')
        joined=row.get('joined_at') or now
        con.execute(text('UPDATE beta_testers SET status=:s,joined_at=:j,last_seen_at=:t,test_count=:c,updated_at=:t WHERE user_id=:u'),{'s':status,'j':joined,'t':now,'c':int(row.get('test_count') or 0)+1,'u':u['id']})
    return jsonify(selected=True,status=status,joined_at=joined,last_seen_at=now)

@app.get('/api/admin/beta-testers')
def admin_beta_testers():
    admin,err=_require_admin()
    if err:return err
    status=str(request.args.get('status') or '').strip();q=str(request.args.get('q') or '').strip().lower()[:120]
    where=[];params={'lim':200}
    if status and status!='all':where.append('b.status=:s');params['s']=status
    if q:
        where.append('(LOWER(u.email) LIKE :q OR LOWER(u.display_name) LIKE :q OR CAST(u.member_number AS TEXT) LIKE :q)');params['q']='%'+q+'%'
    clause=' WHERE '+' AND '.join(where) if where else ''
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text(f'''SELECT b.user_id,b.status,b.note,b.invited_at,b.joined_at,b.last_seen_at,b.completed_at,b.test_count,
          u.member_number,u.email,u.display_name,COALESCE(c.checkin_days,0) AS checkin_days,c.first_checkin_date,c.last_checkin_date
          FROM beta_testers b JOIN users u ON u.id=b.user_id
          LEFT JOIN (SELECT user_id,COUNT(*) AS checkin_days,MIN(checkin_date) AS first_checkin_date,MAX(checkin_date) AS last_checkin_date FROM beta_daily_checkins GROUP BY user_id) c ON c.user_id=b.user_id
          {clause} ORDER BY CASE b.status WHEN 'active' THEN 0 WHEN 'selected' THEN 1 WHEN 'completed' THEN 2 ELSE 3 END,b.updated_at DESC LIMIT :lim'''),params).fetchall()]
        counts={str(r[0]):int(r[1]) for r in con.execute(text('SELECT status,COUNT(*) FROM beta_testers GROUP BY status')).fetchall()}
        eligible=int(con.execute(text('SELECT COUNT(*) FROM (SELECT user_id,COUNT(*) AS cnt FROM beta_daily_checkins GROUP BY user_id HAVING COUNT(*)>=14) q')).scalar() or 0)
    for row in rows:
        row['required_days']=14;row['progress_percent']=min(100,round(int(row.get('checkin_days') or 0)/14*100))
    target=int(_runtime_settings().get('beta_target_count') or 15)
    return jsonify(items=rows,summary={'selected':counts.get('selected',0),'active':counts.get('active',0),'completed':counts.get('completed',0),'removed':counts.get('removed',0),'total':sum(v for k,v in counts.items() if k!='removed'),'target':target,'eligible_14d':eligible})

@app.get('/api/admin/beta-checkins-export.csv')
def admin_beta_checkins_export():
    admin,err=_require_admin()
    if err:return err
    with ENGINE.connect() as con:
        rows=con.execute(text('''SELECT u.member_number,u.email,u.display_name,b.status,c.checkin_date,c.checked_at,c.app_version,c.note
          FROM beta_daily_checkins c JOIN users u ON u.id=c.user_id LEFT JOIN beta_testers b ON b.user_id=c.user_id
          ORDER BY c.checkin_date DESC,u.member_number''')).fetchall()
    out=StringIO();w=csv.writer(out);w.writerow(['회원번호','이메일','이름','테스터상태','확인일','확인시각','앱버전','메모'])
    for r in rows:w.writerow(list(r))
    _audit(admin['id'],'export_beta_checkins',None,f'{len(rows)}건')
    return Response(out.getvalue(),mimetype='text/csv; charset=utf-8',headers={'Content-Disposition':'attachment; filename=resell_pick_beta_checkins.csv','Cache-Control':'no-store'})
...
def admin_release_readiness():
    ...
    if err:return err
    s=_runtime_settings()
    try:saved=json.loads(s.get('release_checklist_json') or '{}')
    except Exception:saved={}
    items=[{'key':key,'label':label,'done':bool(saved.get(key))} for key,label in RELEASE_CHECKLIST_ITEMS]
    return jsonify(items=items,note=s.get('release_checklist_note',''),done_count=sum(1 for x in items if x['done']),total=len(items))

@app.post('/api/admin/release-readiness')
def admin_release_readiness_save():
    admin,err=_require_admin()
    if err:return err
    d=request.get_json(silent=True) or {};raw=d.get('items') if isinstance(d.get('items'),dict) else {};allowed={k for k,_ in RELEASE_CHECKLIST_ITEMS}
    saved={k:bool(v) for k,v in raw.items() if k in allowed};note=str(d.get('note') or '').strip()[:2000]
    _save_runtime_settings({'release_checklist_json':json.dumps(saved,ensure_ascii=False),'release_checklist_note':note},admin['id'])
    _audit(admin['id'],'update_release_readiness',None,json.dumps(saved,ensure_ascii=False))
    return jsonify(ok=True,done_count=sum(1 for v in saved.values() if v),total=len(RELEASE_CHECKLIST_ITEMS))

@app.post('/api/admin/users/<int:uid>/beta-status')
def admin_user_beta_status(uid):
    admin,err=_require_admin()
    if err:return err
    d=request.get_json(silent=True) or {};status=str(d.get('status') or '').lower();note=str(d.get('note') or '').strip()[:1000]
    if status not in {'selected','active','completed','removed'}:return jsonify(error='베타테스터 상태를 확인해 주세요.'),400
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        user=con.execute(text('SELECT id,role FROM users WHERE id=:i'),{'i':uid}).first()
        if not user:return jsonify(error='회원을 찾을 수 없습니다.'),404
        row=con.execute(text('SELECT user_id,invited_at,joined_at FROM beta_testers WHERE user_id=:u'),{'u':uid}).first()
        invited=(row[1] if row else None) or now;joined=(row[2] if row else None)
        if status=='active' and not joined:joined=now
        completed=now if status=='completed' else None
        if row:
            con.execute(text('UPDATE beta_testers SET status=:s,note=:n,invited_at=:i,joined_at=:j,completed_at=:c,updated_by=:a,updated_at=:t WHERE user_id=:u'),{'s':status,'n':note or None,'i':invited,'j':joined,'c':completed,'a':admin['id'],'t':now,'u':uid})
        else:
            con.execute(text('INSERT INTO beta_testers(user_id,status,note,invited_at,joined_at,last_seen_at,completed_at,test_count,updated_by,updated_at) VALUES(:u,:s,:n,:i,:j,NULL,:c,0,:a,:t)'),{'u':uid,'s':status,'n':note or None,'i':invited,'j':joined,'c':completed,'a':admin['id'],'t':now})
    _audit(admin['id'],'update_beta_tester',uid,json.dumps({'status':status,'note':note},ensure_ascii=False))
    return jsonify(ok=True,status=status,updated_at=now)

@app.post('/api/client-errors')
def client_error_create():
    d=request.get_json(silent=True) or {};u=_current_user()
    message=str(d.get('message') or '').strip()
    if len(message)<2:return jsonify(ok=False),202
    try:rid=_record_client_error((u or {}).get('id'),d.get('error_type') or 'javascript',message,d.get('stack'),d.get('source'),d.get('page'),d.get('app_version'),d.get('device'))
    except Exception:
        logging.exception('client error collection failed');return jsonify(ok=False),202
...
def admin_client_errors():
    admin,err=_require_admin()
    if err:return err
    status=str(request.args.get('status') or '').strip();params={'lim':max(1,min(300,int(request.args.get('limit') or 100)))}
    where='';
    if status and status!='all':where=' WHERE e.status=:s';params['s']=status
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text(f'''SELECT e.id,e.user_id,u.member_number,u.email,e.error_type,e.message,e.stack,e.source,e.page,e.app_version,e.device,e.status,e.admin_note,e.occurrence_count,e.first_seen_at,e.last_seen_at
          FROM client_error_logs e LEFT JOIN users u ON u.id=e.user_id {where} ORDER BY CASE e.status WHEN 'new' THEN 0 WHEN 'reviewing' THEN 1 ELSE 2 END,e.last_seen_at DESC LIMIT :lim'''),params).fetchall()]
        counts={str(r[0]):int(r[1]) for r in con.execute(text('SELECT status,COUNT(*) FROM client_error_logs GROUP BY status')).fetchall()}
        last24=(datetime.utcnow()-timedelta(hours=24)).isoformat(timespec='seconds')+'Z'
        recent=int(con.execute(text('SELECT COALESCE(SUM(occurrence_count),0) FROM client_error_logs WHERE last_seen_at>=:c'),{'c':last24}).scalar_one())
    return jsonify(items=rows,summary={'new':counts.get('new',0),'reviewing':counts.get('reviewing',0),'resolved':counts.get('resolved',0),'ignored':counts.get('ignored',0),'recent_24h':recent,'total':sum(counts.values())})

@app.post('/api/admin/client-errors/<int:eid>/status')
def admin_client_error_status(eid):
    admin,err=_require_admin()
    if err:return err
    d=request.get_json(silent=True) or {};status=str(d.get('status') or '').lower();note=str(d.get('admin_note') or '').strip()[:2000]
    if status not in {'new','reviewing','resolved','ignored'}:return jsonify(error='오류 처리 상태를 확인해 주세요.'),400
    now=datetime.utcnow().isoformat(timespec='seconds')+'Z'
    with ENGINE.begin() as con:
        row=con.execute(text('SELECT id FROM client_error_logs WHERE id=:i'),{'i':eid}).first()
        if not row:return jsonify(error='오류 기록을 찾을 수 없습니다.'),404
        con.execute(text('UPDATE client_error_logs SET status=:s,admin_note=:n,updated_at=:t WHERE id=:i'),{'s':status,'n':note or None,'t':now,'i':eid})
    _audit(admin['id'],'update_client_error',None,json.dumps({'error_id':eid,'status':status},ensure_ascii=False))
    return jsonify(ok=True,status=status)

@app.get('/api/admin/client-errors-export.csv')
def admin_client_errors_export():
    admin,err=_require_admin()
    if err:return err
    with ENGINE.connect() as con:
        rows=[dict(r._mapping) for r in con.execute(text('''SELECT e.id,u.member_number,u.email,e.error_type,e.message,e.source,e.page,e.app_version,e.status,e.admin_note,e.occurrence_count,e.first_seen_at,e.last_seen_at
          FROM client_error_logs e LEFT JOIN users u ON u.id=e.user_id ORDER BY e.id DESC''')).fetchall()]
    out=StringIO();out.write('\ufeff');w=csv.writer(out);w.writerow(['번호','회원번호','이메일','종류','오류문구','소스','화면','앱버전','상태','관리자메모','발생횟수','최초발생','최근발생'])
    for r in rows:w.writerow([r.get(k,'') for k in ['id','member_number','email','error_type','message','source','page','app_version','status','admin_note','occurrence_count','first_seen_at','last_seen_at']])
    return Response(out.getvalue(),mimetype='text/csv; charset=utf-8',headers={'Content-Disposition':'attachment; filename=resell_pick_client_errors.csv','Cache-Control':'no-store'})

@app.get('/api/admin/server-usage')
def admin_server_usage():
    admin,err=_require_admin()
    if err:return err
    month=str(request.args.get('month') or datetime.utcnow().strftime('%Y-%m'))[:7];start=month+'-01T00:00:00Z'
    try:
        y,m=[int(x) for x in month.split('-')];end=(datetime(y+1,1,1) if m==12 else datetime(y,m+1,1)).isoformat(timespec='seconds')+'Z'
    except Exception:return jsonify(error='조회 월 형식을 확인해 주세요.'),400
    with ENGINE.connect() as con:
        total=_row_dict(con.execute(text('''SELECT COUNT(*) calls,COALESCE(SUM(input_tokens),0) input_tokens,COALESCE(SUM(output_tokens),0) output_tokens,
          COALESCE(SUM(total_tokens),0) total_tokens,COALESCE(SUM(estimated_usd),0) estimated_usd,COALESCE(SUM(cached),0) cached_calls FROM ai_usage_logs WHERE created_at>=:s AND created_at<:e'''),{'s':start,'e':end}).first()) or {}
        by_kind=[dict(r._mapping) for r in con.execute(text('''SELECT kind,COUNT(*) calls,COALESCE(SUM(total_tokens),0) total_tokens,COALESCE(SUM(estimated_usd),0) estimated_usd FROM ai_usage_logs WHERE created_at>=:s AND created_at<:e GROUP BY kind ORDER BY estimated_usd DESC'''),{'s':start,'e':end}).fetchall()]
        top=[dict(r._mapping) for r in con.execute(text('''SELECT u.member_number,u.email,COUNT(a.id) calls,COALESCE(SUM(a.total_tokens),0) total_tokens,COALESCE(SUM(a.estimated_usd),0) estimated_usd
          FROM ai_usage_logs a LEFT JOIN users u ON u.id=a.user_id WHERE a.created_at>=:s AND a.created_at<:e GROUP BY u.member_number,u.email ORDER BY estimated_usd DESC LIMIT 20'''),{'s':start,'e':end}).fetchall()]
        analysis=int(con.execute(text('SELECT COALESCE(SUM(analysis_count),0) FROM monthly_usage WHERE month_key=:m'),{'m':month}).scalar_one())
    return jsonify(month=month,total=total,by_kind=by_kind,top_users=top,analysis_count=analysis)
    ...
    try:
        ...
        for model in candidates:
            ...
            try:
                ...
                data=parse(r.output_text);data['_api_usage']=_api_usage_meta(r,model,'vision');_record_ai_usage(reserved_user,data['_api_usage']);_cache_set(cache_key,data);return data,None
...
def server_error(e):
    try:
        u=_current_user();_record_client_error((u or {}).get('id'),'server_500',str(e),source=request.path,page=request.path,app_version='6.19.0',device=request.headers.get('User-Agent',''))
    except Exception:logging.exception('server error logging failed')
...
@app.get('/privacy')
def public_privacy():return send_from_directory('.','privacy.html')
@app.get('/terms')
def public_terms():return send_from_directory('.','terms.html')
@app.get('/account-delete')
def public_account_delete_page():return send_from_directory('.','account-delete.html')
@app.get('/<path:p>')
...
{{"product_name":"","brand":"Nike|New Balance|Adidas|Under Armour|기타","model_no":"","color":"","release_price":0,"sizes":[{{"size":0,"highest_bid":0,"lowest_ask":0,"recent_price":0,"recent_date":"","trade_count":0,"trades":[{{"date":"YYYY-MM-DD","price":0}}]}}],"summary":"공개정보 기반 핵심 판단 한두 문장","confidence":"높음|보통|낮음"}}"""
        ...
        d['_api_usage']=_api_usage_meta(response,model,'web_search');_record_ai_usage(_current_user(),d['_api_usage'])
...
{{"main_keyword":"","related_keywords":[],"demand_score":0,"competition_score":0,"sourcing_score":0,"turnover":"빠름|보통|느림|자료부족","recommendation":"적극 소싱|마진 확보 시 소싱|소량 테스트|비추천|자료부족","seller_count":0,"product_count":0,"search_trend":"상승|보합|하락|판단어려움","trend_reason":"","review_signal":"","purchase_signal":"","price_competition":"낮음|보통|높음|판단어려움","price_range_note":"","confidence":"높음|보통|낮음","confidence_reason":"","evidence":[],"cautions":[]}}"""
    ...
    try:
        ...
        d=parse(response.output_text)
        d['_api_usage']=_api_usage_meta(response,model,'keyword_search');_record_ai_usage(reserved_user,d['_api_usage'])
    except Exception as exc:
        if reserved_user:_rollback_analysis(reserved_user['id'])
...
def health():
    try:
        with ENGINE.connect() as con:con.execute(text('SELECT 1')).scalar_one()
        return jsonify(ok=True,version='6.19.0',database='postgresql' if DB_URL.startswith('postgresql') else 'sqlite')
    except Exception as exc:
        logging.exception('health database check failed')
        return jsonify(ok=False,version='6.19.0',database='unavailable',error='database connection failed'),503
