...
def _init_db():
    ...
    with ENGINE.begin() as con:
        for q in (users_sql,snapshot_sql,usage_sql,reg_sql,audit_sql): con.execute(text(q))
        if DB_URL.startswith('postgresql'):
            ...
            additions={'email_key':'VARCHAR(255)','email_verified':'INTEGER NOT NULL DEFAULT 0','verification_code_hash':'VARCHAR(128)','verification_expires_at':'VARCHAR(40)','registration_ip_hash':'VARCHAR(128)','reset_code_hash':'VARCHAR(128)','reset_expires_at':'VARCHAR(40)','reset_requested_at':'VARCHAR(40)','auth_version':'INTEGER NOT NULL DEFAULT 1','plan_started_at':'VARCHAR(40)','plan_expires_at':'VARCHAR(40)','role':"VARCHAR(20) NOT NULL DEFAULT 'user'",'account_status':"VARCHAR(20) NOT NULL DEFAULT 'active'"}
...
def _current_user():
    ...
    with ENGINE.connect() as con:
        row=_row_dict(con.execute(text('SELECT id,email,display_name,plan,created_at,last_login_at,email_verified,auth_version,plan_started_at,plan_expires_at,role,account_status FROM users WHERE id=:id'),{'id':uid}).first())
    if row:
...
def admin_users():
    ...
    where=''
    if q:
        where='WHERE LOWER(email) LIKE :q OR LOWER(display_name) LIKE :q'
        params['q']='%'+q+'%'
    sql=f'''SELECT id,email,display_name,plan,role,account_status,email_verified,created_at,last_login_at,plan_started_at,plan_expires_at
