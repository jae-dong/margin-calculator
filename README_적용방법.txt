...
3. Summary에 '리셀 PICK v6.2.1 배포 안정화'를 입력합니다.
...
6. 배포 성공 후 웹주소 뒤에 /health를 붙여 ok:true, version:6.2.1, database:postgresql이 표시되는지 확인합니다.
...
[v6.2.1 주요 변경]
- 로그인 화면에 비밀번호 찾기 추가
- 가입 이메일로 6자리 재설정 인증번호 발송
- 인증번호 10분 만료 및 60초 재요청 제한
- 비밀번호 재설정 시 기존 로그인 세션 자동 무효화
- 이용 상태·이용 기간 글자 대비 개선
- 결제 이용권의 만료일과 남은 사용일수 표시
- 유료 이용권 만료 시 무료 제공량으로 자동 전환

[결제 이용기간 데이터]
실제 Google Play 결제가 연결되면 서버가 users.plan_started_at과 users.plan_expires_at을 갱신해야 합니다.
현재 결제 연동 전에는 화면에 '결제 전'으로 표시됩니다.
